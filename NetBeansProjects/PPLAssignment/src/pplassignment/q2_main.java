...
public class q2_main {
    public static void main(String[] args) throws IOException {
        ...
        int noOfGirl = rand.nextInt(15) + 5;
        int noOfBoy = (rand.nextInt(3)+2) * noOfGirl;        
        int noOfGift = rand.nextInt(20) + 15,i;
	  ...
	  if (!file1.exists()) {
	     ...
	     file2.createNewFile();
            ...
            for(i = 0; i< noOfGift; i++){
                br = new String(arr_gifts[i].gifttype + " " +arr_gifts[i].price +" "+ arr_gifts[i].value + " "+arr_gifts[i].luxRating+" "+ arr_gifts[i].difficultyObtain + " "+arr_gifts[i].utilClass+" "+ arr_gifts[i].utilValue);
                bw2.write(br);
                bw2.newLine();
